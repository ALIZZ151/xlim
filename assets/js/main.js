...
function initPreloader() {
  window.addEventListener('load', () => {
    ...
    const interval = setInterval(() => {
      ...
      if (progress >= 100) {
        progress = 100;
        if (progressEl) progressEl.textContent = '100% - STORE READY';
        ...
        setTimeout(() => {
          ...
          if (window.AOS) AOS.init({ once: true, duration: 800, offset: 50, easing: 'ease-out-cubic' });
          if (window.gsap && document.querySelector('.gs-reveal')) {
            gsap.from('.gs-reveal', { y: 40, opacity: 0, duration: 1, stagger: 0.14, ease: 'power3.out', delay: 0.1 });
          }
        }, 350);
      } else if (progressEl) {
...
function initNavbar() {
  ...
  window.addEventListener('scroll', () => {
    ...
    if (window.scrollY > 50) {
      navbar.style.background = 'rgba(2,6,23,.88)';
      navbar.style.borderBottom = '1px solid rgba(14,165,233,.22)';
      backToTop?.classList.add('show');
    } else {
      navbar.style.background = 'rgba(2,6,23,.72)';
      navbar.style.borderBottom = '1px solid rgba(148,163,184,.12)';
...
export async function loginWithGoogle() {
  ...
  if (error) window.Swal?.fire({ icon: 'error', title: 'Login gagal', text: error.message, background: '#07111f', color: '#fff' });
}
...
export async function updateUserUI() {
  ...
  loginBtns.forEach((btn) => btn.style.display = user ? 'none' : 'inline-flex');
  logoutBtns.forEach((btn) => btn.style.display = user ? 'inline-flex' : 'none');
  userPills.forEach((pill) => pill.style.display = user ? 'inline-flex' : 'none');
  ...
  if (user) {
    ...
    userAvatars.forEach((img) => { img.src = avatar; img.alt = name; });
  }
}
